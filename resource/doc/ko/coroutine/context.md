`support\Context` 클래스는 요청 컨텍스트 데이터를 저장하는 데 사용되며, 요청이 완료되면 해당 컨텍스트 데이터가 자동으로 삭제됩니다. 즉, 컨텍스트 데이터 수명은 요청 수명에 따라 지속됩니다. `support\Context`는 Fiber, Swoole, Swow 코루틴 환경을 지원합니다.
