...
├── config                        설정 디렉토리
│   ├── app.php                   어플리케이션 설정
│   ├── autoload.php              여기에 있는 파일은 자동으로 로드됨
│   ├── bootstrap.php             프로세스 시작 시 onWorkerStart에서 실행되는 콜백 설정
│   ├── container.php             컨테이너 설정
│   ├── dependence.php            컨테이너 종속성 설정
│   ├── database.php              데이터베이스 설정
│   ├── exception.php             예외 설정
│   ├── log.php                   로그 설정
│   ├── middleware.php            미들웨어 설정
│   ├── process.php               사용자 정의 프로세스 설정
│   ├── redis.php                 레디스 설정
│   ├── route.php                 라우트 설정
│   ├── server.php                포트, 프로세스 수 등 서버 설정
│   ├── view.php                  뷰 설정
│   ├── static.php                정적 파일 스위치 및 정적 파일 미들웨어 설정
│   ├── translation.php           다국어 설정
│   └── session.php               세션 설정
├── public                        정적 자원 디렉토리
├── process                       사용자 정의 프로세스 디렉토리
├── runtime                       어플리케이션의 런타임 디렉토리, 쓰기 권한이 필요함
├── start.php                     서비스 시작 파일
...
└── support                       라이브러리 어댑터(서드파티 라이브러리 포함)
    ...
    ├── helpers.php               도우미 함수(app/functions.php에 비즈니스 맞춤 함수 작성)
